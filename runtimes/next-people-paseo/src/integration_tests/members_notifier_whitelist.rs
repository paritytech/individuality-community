...
//! Asset hub's genesis subscription whitelist on `MembersNotifier`.

use super::*;
use crate::people::NEXT_ASSET_HUB_MEMBERS_SUBSCRIBER_INDEX;
...
use paseo_runtime_constants::system_parachain::NEXT_ASSET_HUB_ID;

fn asset_hub() -> ParaId {
	ParaId::from(NEXT_ASSET_HUB_ID)
}
...
fn every_configured_whitelist_entry_is_well_formed() {
	let entries = crate::people::asset_hub_subscription_whitelist();
...
fn genesis_whitelists_asset_hub() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(entry.pallet_index, NEXT_ASSET_HUB_MEMBERS_SUBSCRIBER_INDEX);
...
fn anyone_can_subscribe_asset_hub_without_governance() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(info.pallet_index, NEXT_ASSET_HUB_MEMBERS_SUBSCRIBER_INDEX);
		...
		assert_eq!(pending.pallet_index, NEXT_ASSET_HUB_MEMBERS_SUBSCRIBER_INDEX);
...
fn root_unsubscribing_asset_hub_is_final() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(MembersNotifier::subscribe(
			...
			NEXT_ASSET_HUB_MEMBERS_SUBSCRIBER_INDEX,
		));
		assert!(Subscribers::<Runtime>::contains_key(asset_hub()));
	});
}
