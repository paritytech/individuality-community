...
/// ## Example
/// ```
/// use next_asset_hub_paseo_runtime::staking::{GeometricDeposit, InitialBaseDeposit};
...
/// let deposit = next_asset_hub_paseo_runtime::staking::SignedDepositPerPage::get() * 16;
...
impl pallet_staking_async::Config for Runtime {
	...
	// Issuance is now driven by `pallet_dap::IssuanceCurve` (see `IssuanceCurve` below).
	// `EraPayout` is left at the unit type, matching Westend's DAP-driven setup. The legacy
	// `EraPayout` struct is kept for `impl_experimental_inflation_info` (informational API).
	type EraPayout = ();
	...
	type StakerRewardCalculator = pallet_staking_async::reward::DefaultStakerRewardCalculator<Self>;
...
impl pallet_staking_async_rc_client::Config for Runtime {
	...
	type WeightInfo = pallet_staking_async_rc_client::weights::SubstrateWeight<Runtime>;
}

/// Yearly emission curve for DAP. Mirrors Westend's structure (flat 8% of a fixed
/// reference TI), but uses Paseo's pre-hard-cap TI as the reference. The hard-cap
/// stepped curve from `EraPayout::yearly_after_hard_cap` is *not* reflected here yet —
/// switching to DAP-based issuance superseded the era-payout pathway.
...
impl frame_support::traits::OnRuntimeUpgrade for InitiateStakingAsync {
	fn on_runtime_upgrade() -> Weight {
		...
		// Set the minimum score floor for the election.
		//
		// Sized for a testnet with 7 validators with 100 units of self-stake, taking as reference
		// the solution built by offchain workers with `minimal_stake = 100 units`; we set the
		// floor to ~50% of that.
		let minimum_score = sp_npos_elections::ElectionScore {
			minimal_stake: 500_000_000_000, // 50 units
			sum_stake: 3_500_000_000_000,   // 350 units
			sum_stake_squared: 14_000_000_000_000_000_000_000_000,
		};
