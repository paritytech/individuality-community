...
#
# Vendored from paritytech/polkadot-bulletin-chain (scripts/get_polkadot_binaries.sh), with one local
# change: honour FORCE_SOURCE_BUILD=1 so a non-commit ref (a release tag) is BUILT from source rather
# than downloaded — our `*-local` relay needs the `fast_runtime_binary` that prebuilt releases omit.
# Driven by 00-install-binaries.sh, which feeds it the .github/env release tag as POLKADOT_NODE_VERSION.
...
#   zombienet-sdk        → zombie-cli (release-tag only; source-build not supported)
#
...
#   TRY_RUNTIME_VERSION, ZOMBIENET_SDK_VERSION
#
# Value semantics:
...
configure_group() {
	case "$1" in
		...
		try-runtime)
			...
			;;
		zombienet-sdk)
			VERSION_VAR="ZOMBIENET_SDK_VERSION"
			BINARIES="zombie-cli"
			RELEASE_REPO="paritytech/zombienet-sdk"
			SOURCE_URL=""
			SOURCE_DIR=""
			;;
		*)
			die "unknown group: $1 (expected one of: polkadot-node, frame-omni-bencher, chain-spec-builder, try-runtime, zombienet-sdk)"
			;;
	esac
}
...
release_asset_name() {
	...
	case "$group" in
		...
		zombienet-sdk)
			case "$PLATFORM" in
				linux-x86_64)   echo "zombie-cli-x86_64-unknown-linux-musl" ;;
				darwin-aarch64) echo "zombie-cli-aarch64-apple-darwin" ;;
			esac
			;;
	esac
}
...
# FORCE_SOURCE_BUILD=1 builds a non-commit ref (e.g. a release tag) from source — local addition.
if [ "${FORCE_SOURCE_BUILD:-0}" = 1 ] || [[ "$REF" =~ ^[0-9a-fA-F]{40}$ ]]; then
	MODE=source
else
	MODE=release
fi
