...
- Gate the read-only check on the selector, not on the whole precompile: `if env.is_read_only() && is_mutating(input)`, as in the `scarcity` collection and `nft-claims` precompiles. A view selector must stay callable from a read-only frame. A blanket `env.is_read_only()` denial is correct only when every selector mutates, as in the `scarcity` factory.
- Resolve the caller with `caller_account`, not by hand. It reverts with `ERR_INVALID_CALLER` when the caller is a `Root` origin, which has no account id.
