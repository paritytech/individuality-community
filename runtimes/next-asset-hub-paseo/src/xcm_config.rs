...
use super::{
	genesis_config_presets::EXTERNAL_ASSET_ID, treasury, AccountId, AllPalletsWithSystem,
	AssetConversion, Assets, Balance, Balances, CollatorSelection, ForeignAssets, GeneralAdmin,
	NativeAndAssets, ParachainInfo, ParachainSystem, PaseoWeightToFee as WeightToFee, PolkadotXcm,
...
pub type TreasuryAccount = treasury::TreasuryAccount;
...
pub type XcmOriginToTransactDispatchOrigin = (
	...
	// transaction from the Root origin. Required for `pallet-staking-async-rc-client` to accept
	// session reports and offences sent by the relay chain via `Transact { Superuser, .. }`.
...
parameter_types! {
	/// AH-local view of an external asset. Mirrors `ExternalAssetLocation` in
	/// `next-people-paseo`, re-anchored to this chain.
...
/// - Sibling parachains' assets from where they originate (as `ForeignCreators`); and
...
type LocalXcmRouter = LocalXcmRouterWithoutException;

/// Routes non-local XCM messages.
...
pub mod bridging {
	...
	parameter_types! {
		...
		pub storage XcmBridgeHubRouterBaseFee: Balance =
			bp_bridge_hub_paseo::estimate_paseo_to_kusama_message_fee(0);
