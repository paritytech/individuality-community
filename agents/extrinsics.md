...
- For a struct or enum stored in storage, use `#[frame_support::stored]` instead of hand-listing `Encode, Decode, DecodeWithMemTracking, MaxEncodedLen, TypeInfo`, the `*NoBound` derives and `#[scale_info(skip_type_params(..))]`. It bounds on field types, so a type generic over `T: Config` needs no manual bounds. It does not derive `Default`, `Ord` or `PartialOrd`: add `DefaultNoBound` alongside it when the type backs a `ValueQuery`. Apply it to new types; do not convert existing ones, because adding `skip_type_params` where it was absent changes the metadata.
