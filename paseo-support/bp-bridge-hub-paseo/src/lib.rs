...
/// Kusama uses 12 decimal places, same as Polkadot/Paseo.
const KUSAMA_UNITS: Balance = 1_000_000_000_000;
...
fn convert_from_uksm_to_udot(price_in_uksm: Balance) -> Balance {
	...
	let nominal_ratio =
		FixedU128::from_rational(paseo_runtime_constants::currency::UNITS, KUSAMA_UNITS);
...
mod tests {
	...
	fn convert_from_uksm_to_udot_works() {
		let price_in_uksm = 77 * KUSAMA_UNITS;
		let same_price_in_udot = convert_from_uksm_to_udot(price_in_uksm);

		let price_in_ksm = FixedU128::from_rational(price_in_uksm, KUSAMA_UNITS);
		let price_in_dot =
