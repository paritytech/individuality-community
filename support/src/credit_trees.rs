...
const _: () = {
	// A root removed at or before its tree leaves a credit inside its claim deadline with no awards
	// to build a proof from.
	assert!(ROOT_TTL_GRACE > 0);
};
...
{
	Next::mutate(|next| {
		*next = Some(next.map_or(bucket, |next| next.min(bucket)));
	});
}
