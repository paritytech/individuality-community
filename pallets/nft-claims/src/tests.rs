...
mod claim {
	...
	fn the_last_claim_of_a_tree_removes_it_and_queues_its_deletion() {
		new_test_ext().execute_with(|| {
			...
			// The spent leaves outlive the tree, so a replay that puts it back mints nothing.
			for (claimant, credit) in &awards {
				assert!(ClaimedCredits::<Test>::contains_key(BLOCK, credit_leaf(claimant, credit)));
			}
	...
	#[test]
	fn a_sweep_removes_a_partly_claimed_tree_and_keeps_its_spent_leaves() {
		new_test_ext().execute_with(|| {
			let awards = awards();
			let tree = tree_of(BLOCK, &awards);
			store_tree(&awards);
			...
			set_now(bucket_due_at(expiry_bucket(tree.timestamp)));
			...
			// A replay on the game chain can deliver the tree again, and only the spent leaves
			// keep its claimed credits from minting a second time, so the sweep leaves them.
			assert_eq!(ClaimedCounts::<Test>::get(BLOCK), 1);
			assert!(ClaimedCredits::<Test>::contains_key(
				BLOCK,
				credit_leaf(&AccountOrPerson::Account(ALICE), &[1u8; 32])
			));
		});
	}

	#[test]
	fn a_leaf_at_a_high_index_claims_on_its_own() {
		new_test_ext().execute_with(|| {
			// Nine awards, so the claimed one is not the first.
			let awards = (1..=9u8)
			...
			assert!(ClaimedCredits::<Test>::contains_key(
				BLOCK,
				credit_leaf(&AccountOrPerson::Account(ALICE), &[9u8; 32])
			));
			assert_eq!(ClaimedCounts::<Test>::get(BLOCK), 1);
			assert!(
				!ClaimedCredits::<Test>::contains_key(
					BLOCK,
					credit_leaf(&AccountOrPerson::Account(ALICE), &[1u8; 32])
				),
				"the other leaves stay unspent"
			);
		});
	}

	#[test]
	fn a_sweep_of_a_fully_claimed_blocks_bucket_sends_no_second_deletion() {
		new_test_ext().execute_with(|| {
			...
			assert!(!nft_claims_events().contains(&Event::CreditTreesExpired {
				bucket: expiry_bucket(tree.timestamp),
				count: 1
			}));
		});
	}
	...
	fn a_minter_reentering_with_another_credit_loses_neither_claims_count() {
		new_test_ext().execute_with(|| {
			...
			assert!(ClaimedCredits::<Test>::contains_key(
				BLOCK,
				credit_leaf(&AccountOrPerson::Account(ALICE), &[3u8; 32])
			));
		});
	}
	...
	fn try_state_holds_after_claims_and_catches_corrupted_accounting() {
		new_test_ext().execute_with(|| {
			...
			ClaimedCounts::<Test>::insert(BLOCK, 4);
	...
	/// A block with spent leaves and no tree is what a removal leaves behind, so `try_state`
	/// accepts it. A tree that is still held without its bucket entry is the state that would
	/// keep it past its deadline, so `try_state` rejects that.
	#[test]
	fn try_state_holds_once_a_tree_is_removed_and_catches_an_unfiled_tree() {
		new_test_ext().execute_with(|| {
			...
			// The removal takes the tree and its expiry entry, leaving the spent leaves alone.
			assert!(!CreditTrees::<Test>::contains_key(BLOCK));
			assert!(!TreeExpiries::<Test>::contains_key(expiry_bucket(tree.timestamp), BLOCK));
			assert_eq!(ClaimedCounts::<Test>::get(BLOCK), 1);
			assert_ok!(NftClaims::do_try_state());

			// A held tree that no bucket names is one no sweep reaches.
			store_tree(&awards);
			assert_ok!(NftClaims::do_try_state());
...
mod expiry {
	...
	use indiv_support::offchain::RETRY_WINDOW;
	...
	fn a_tree_arriving_in_an_earlier_bucket_lowers_the_watermark() {
		new_test_ext().execute_with(|| {
			...
			crate::Pallet::<Test>::note_expiry(late, tree(late).timestamp);
	...
	/// The window, not the block, is what the discriminator names: two blocks of one window encode
	/// the same call, which is what lets the pool drop the repeat.
	#[test]
	fn the_offchain_worker_names_the_retry_window_of_the_submitting_block() {
		new_test_ext().execute_with(|| {
			assert_ok!(NftClaims::receive_credit_trees(
				game_chain_origin(),
				batch(vec![update(0, 10)])
			...
			run_offchain_worker(RETRY_WINDOW as u64);
...
mod deletions {
	...
	fn send() -> frame_support::dispatch::DispatchResultWithPostInfo {
		NftClaims::send_tree_deletions(RuntimeOrigin::from(frame_system::RawOrigin::Authorized), 1)
	}
	...
	#[test]
	fn a_full_queue_drops_the_deletion_and_reports_it() {
		new_test_ext().execute_with(|| {
			// A queue at `MaxQueuedTreeDeletions`. One sweep cannot fill it that far, but a stalled
			// delivery leaves it in this state.
			queue(vec![1, 2, 3, 4]);

			crate::Pallet::<Test>::queue_tree_deletions(&[5]);

			assert_eq!(PendingTreeDeletions::<Test>::get().to_vec(), vec![1, 2, 3, 4]);
			assert!(nft_claims_events().contains(&Event::TreeDeletionDropped { block: 5 }));
		});
	}
	...
	fn a_send_is_authorized_only_while_something_is_queued() {
		new_test_ext().execute_with(|| {
			assert_eq!(
				crate::Pallet::<Test>::authorize_send_tree_deletions(TransactionSource::Local),
				Err(TransactionValidityError::Invalid(InvalidTransaction::Custom(
			...
			assert_eq!(
				crate::Pallet::<Test>::authorize_send_tree_deletions(TransactionSource::External),
				Err(TransactionValidityError::Invalid(InvalidTransaction::Custom(
					AuthorizeInvalidity::TransactionNotLocal as u8
				))),
			);
	...
	/// The send carries the window alone, so a retry inside one window repeats the call the pool
	/// already holds and a new window gives it a hash the pool has not banned.
	#[test]
	fn the_offchain_worker_names_the_retry_window_of_the_submitting_block() {
		new_test_ext().execute_with(|| {
			queue(vec![10, 11, 12]);
			...
			assert_eq!(
				submitted_calls(),
				vec![
					RuntimeCall::NftClaims(crate::Call::send_tree_deletions { discriminator: 0 }),
					RuntimeCall::NftClaims(crate::Call::send_tree_deletions { discriminator: 1 }),
				]
			);
		});
	}
