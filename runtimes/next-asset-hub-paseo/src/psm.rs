...
//! Fungibles adapter for PSM, copied from Asset Hub Polkadot.
//!
//! `pallet_psm::EnsureAssetOwner` needs `fungibles::roles::Inspect` on `Config::Fungibles`, which
//! `frame_support::traits::fungibles::UnionOf` does not implement. [`UnionOf`] here adds it.
// TODO: Remove once frame-support ships `fungibles::roles::Inspect` for `UnionOf`.
...
/// [`frame_support::traits::fungibles::UnionOf`] with `fungibles::roles::Inspect` added. `Inspect`
/// and `metadata::Inspect` forward to the inner union; the other impls dispatch on `Criterion`.
