...
fn test_buy_and_refund_weight_with_native() {
	ExtBuilder::<Runtime>::default()
		...
		.execute_with(|| {
			...
			// Withdraw from bob to create a proper AssetsInHolding with imbalances.
...
fn test_buy_and_refund_weight_with_swap_local_asset_xcm_trader() {
	ExtBuilder::<Runtime>::default()
		...
		.execute_with(|| {
			...
			// keep initial native total issuance to assert later (swap doesn't mint native).
			...
			// Record asset total_issuance after withdraw for accurate final comparison.
			...
			drop(trader);
			assert_eq!(Balances::balance(&staking_pot), initial_balance + fee - refund);
			// With imbalance accounting, asset total_issuance should match what it was after
			// withdraw.
