...
A claim path that breaks the link between the player who earned an NFT claim credit and the account
that mints it. A game opts into private claims at scheduling time.
...
- `close_private_ring`: authorized call submitted by the pallet's offchain worker, once the
  ...
  as closed in `PrivateGameEnds`. It only reclaims space: a closed window takes no claim, so it runs
...
once. `tier` is a public call argument and names a context.
...
The delay opens every member's claims in the same block, so a wallet that watches the chain closely
gains no head start. The close bounds the interval a game's claims fall in.
...
ring 1 and mints at least once. The cost is in credits, because a person claimant has no account
on the People chain to burn PGAS from. One credit is one attestation, so a ring key
...
claimant spends up the ladder and stops where the set gets too small for them.
...
retried. The other integrity tests assert that the claim window is at least one
...
- **A game's registration is capped at `MaxPrivateRingKeys`.** Ring 1 holds every registrant, so
  the cap covers the whole game. Registration is first-come, first-served, and a game with more
  claimants than one ring holds turns the surplus away, leaving them no private path.
  `PrivateRingExponent` is what lifts the cap. Splitting a tier over several rings would leave each
  claim a smaller set to hide in.
- **A higher tier is a smaller set.** The ladder is nested, so the sets shrink as a claimant climbs
  ...
  spends the whole ladder is as anonymous as its top tier makes them, and one that stops lower
  keeps the wider set and forfeits the rest.
- **The floor counts keys.** Registrants who collude, or who register a key they never claim with,
  count towards it while hiding nobody, and an unused key looks the same as a used one. Duplicate
  keys are refused. Registration costs credits the claimant earned in that game, and a key that
  pads tier `t` has to carry `t` of them, so the smallest sets cost the most to pad.
  `MinPrivateRingParticipation` makes the number scale with the game. Both raise the price of
  padding a set.
- **The ring's guarantee ends at the mint.** A Scarcity purse key holds one NFT, so a claim into a
  key that holds one already fails and every claim mints into a key of its own. What happens to
  those keys afterwards is outside the ring: moving the NFTs out of them, or deriving them so that
  an observer can enumerate them, links the claims again.
...
- **Decide how far up the ladder to spend.** Claiming tier 1 alone keeps the widest crowd and
  forfeits the rest. Claiming every tier accepts the top tier's set for the claim that names it.
- **Mint each claim into a fresh purse key.** The chain forces a fresh key by refusing one that
  holds an NFT already. The keys also have to be underivable from one another and left alone
  afterwards.
